 ...
 * Program: Creates and starts two tasks (initialize and datacollect), sets up an ISR catch, then suspends self.
 * Important Aspects:
 * - Can change the priority of tasks
 * - Configuration Information
 * - ISR catch information
 */
...
rtems_task Init(rtems_task_argument ignored){
    ...
    // ISR to catch the timer interrupt
    sc = rtems_interrupt_catch(
            ISRroutine, // Name of ISR function
            83, // Vector address
            &old_ISR); // Unused

    if (sc != RTEMS_SUCCESSFUL) { // Checks for errors
        printf("Can't create ISR Routine: %s\n", rtems_status_text(sc));
        rtems_task_suspend(RTEMS_SELF);
    }
    // Set level and priority of rtems catch
    rtems_interrupt_level level = 2;
    rtems_interrupt_level priority = 5;

    sc = bsp_allocate_interrupt(level,priority);
    if (sc != RTEMS_SUCCESSFUL) { // Checks for errors
        printf("Can't allocate interrupt: %s\n", rtems_status_text(sc));
        rtems_task_suspend(RTEMS_SELF);
    }
    // Sets the mask for the ISR
    MCF5282_INTC0_ICR19 = MCF5282_INTC_ICR_IL(level) | MCF5282_INTC_ICR_IP(priority);
    MCF5282_INTC0_IMRL &= ~(MCF5282_INTC_IMRL_INT19 | MCF5282_INTC_IMRL_MASKALL);
